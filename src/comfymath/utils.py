...
download_history = []
def try_download_video(url, out_name):
    global download_history
    print("ytdl_path state", ytdl_path, folder_paths.get_temp_directory())
    if ytdl_path is None:
        return None
    
    #  todo: 如果要判断，那么需要增加一个超过时间清理动作，因为url内容可能会更改， 还要增加一个重启feed download_history动作
    # found = [item for item in download_history if item.get('url') == url]
    # if url in download_history:
    #     return download_history[url]
    # if len(download_history) > 50:
    #     os.remove(download_history[0].url)
    #     download_history = download_history[(len(download_history)-50):]
    ...
    print("video downlaod temp folder", folder_paths.get_temp_directory())
    try:
        ...
        res = subprocess.run(['curl', url, '-o', "{}/{}".format(folder_paths.get_temp_directory(), out_name)],
                             capture_output=True, check=True)
        #strip newline
        # file = res.stdout.decode('utf-8')[:-1]
        file = "{}/{}".format(folder_paths.get_temp_directory(), out_name)
    except subprocess.CalledProcessError as e:
        raise Exception("An error occurred in the yt-dl process:\n" \
                + e.stderr.decode("utf-8"))
        file = None
    print("download result", file)
    # download_history.append({ "url": file})
    return file
